    ...
    : QMainWindow(parent)
{
    ...
    // Game State Control Menu items
    QMenu *gState = new QMenu("&Game");
    gStart = new QAction("&StartGame", gState);
    gStop  = new QAction("S&top Game", gState);
    gState->addAction(gStart);
    gState->addAction(gStop);
    simulatorMenu->addMenu(robotMenu);
    simulatorMenu->addMenu(ballMenu);
    menuBar()->addMenu(gState);
    ...
    //Game Control
    QObject::connect(gStart, SIGNAL(triggered()), this, SLOT(startGame()));
    QObject::connect(gStop,  SIGNAL(triggered()), this, SLOT(stopGame()));
    startGame();
...
{
    for(int i = 0; i<ROBOT_COUNT*2; i++) {
        glwidget->ssl->robots[i]->on = false;
    }
