...
void Robot::Kicker::step()
{
    ...
    else if (rolling!=0)
    {
        ...
        if (isTouchingBall())
        {
            //float f1x,f1y,f1z;
            float bx,by,bz;
            //rob->chassis->getBodyDirection(f1x,f1y,f1z);
            rob->getBall()->getBodyPosition(bx,by,bz);
            float botx, boty;
            rob->getXY(botx, boty);
            float fx = bx-botx, fy = by-boty;
            float fz = sqrt(fx*fx + fy*fy); // Increasing the power of dribbler.
            fx/=fz;fy/=fz;
            ...
            float dir = 1;
            if (yy>0) dir = -1.0f;
            dBodySetAngularVel(rob->getBall()->body,fy*rob->cfg->robotSettings.RollerTorqueFactor*1500,-fx*rob->cfg->robotSettings.RollerTorqueFactor*1500,0);
            //dBodyAddTorque(rob->getBall()->body,fy*rob->cfg->ROLLERTORQUEFACTOR(),-fx*rob->cfg->ROLLERTORQUEFACTOR(),0);
            //dBodyAddTorque(rob->getBall()->body,yy*fx*rob->cfg->robotSettings.RollerPerpendicularTorqueFactor,yy*fy*rob->cfg->robotSettings.RollerPerpendicularTorqueFactor,vz*1000);
        }
    }
