        ...
        if (size > 0)
        {
            ...
            if(packet.has_debuginfo())
            {
                ...
                for(int i=0; i<d.circle_size(); ++i) {
                    g->debugger.addCircle(d.circle(i).x(), d.circle(i).y(), d.circle(i).radius(), (unsigned int)d.circle(i).color());
                }
                for(int i=0; i<d.line_size(); ++i) {
                    g->debugger.addLine(d.line(i).x1(),d.line(i).y1(), d.line(i).x2(), d.line(i).y2(), (unsigned int)d.line(i).color());
                }
                if(packet.debuginfo().botpos_size() > 0)
                {
                    for(int i=0; i < d.botpos_size(); i++)
                    {
                        qDebug()<<i<<" "<<d.botpos(i).x()/1000.0<< " "<< d.botpos(i).y()/1000.0;
