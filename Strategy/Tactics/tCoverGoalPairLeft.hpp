	 ...
	 int chooseBestBot(std::list<int>& freeBots, const Tactic::Param* tParam, int prevID) const
		{
			...
			for (std::list<int>::iterator it = freeBots.begin(); it != freeBots.end(); ++it)
			{
				...
				if(ForwardX(state->ballPos.x) < (-(HALF_FIELD_MAXX-GOAL_DEPTH)*0.8 + 2*BOT_RADIUS) && (state->ballPos.y >OUR_GOAL_MAXY))
				{
...
*/
   ...
   int nearestOppBot(int destx)
   {
	...
	case DEFENDING :
		...
		if(ForwardX(state->ballPos.x) < ForwardX(dest.x + 2*BOT_RADIUS) && (state->ballPos.y >OUR_GOAL_MAXY))
		{
		   iState = CLEARING ;
		   break ;
		}
